...
class VideoServer:
    def __init__(self, video_dir: str = "videos"):
        ...
        self.preview_frame = None
        self.preview_lock = threading.Lock()
        self.restart_lock = threading.Lock()  # Added lock for video restart
        ...
        # Attach event listener for loop playback
    ...
    def _restart_video(self, event):
        ...
        with self.restart_lock:  # Prevent multiple simultaneous restarts
            if self.current_video and self.is_playing:
                logger.info(f"Restarting video: {self.current_video}")
                self.player.stop()  # Ensure clean state before restart
    ...
    def load_last_played(self):
        ...
        try:
            if self.last_played_file.exists():
                with open(self.last_played_file, "r") as f:
                    data = json.load(f)
                    if "last_video" in data and data["last_video"]:
                        video_path = self.video_dir / data["last_video"]
                        if video_path.exists():  # Only play if file still exists
                            self.play_video(data["last_video"])
        except Exception as e:
            logger.error(f"Error loading last played video: {e}")
    ...
    def delete_video(self, video_name: str) -> bool:
        ...
        if self.current_video == video_name:
            self.stop_video()
            self.current_video = None
            self.save_last_played()  # Update last played file when deleting current video
...
if __name__ == "__main__":
    ...
    try:
        ...
        uvicorn.run(
            "server:app", host="0.0.0.0", port=8000, reload=True, access_log=False
        )
    finally:
